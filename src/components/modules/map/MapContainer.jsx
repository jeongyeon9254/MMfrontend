...
const MapContainer = () => {
  ...
  useEffect(async() => {
    // 첫 요청
    if(list.gps === null) {
      await axios.get('https://run.mocky.io/v3/1ed4adfb-c9fd-44a8-b995-f25dfe6fb6ae'
      ).then((response) => {
  ...
  const _map_1 =() => {
    axios.get('https://run.mocky.io/v3/1ed4adfb-c9fd-44a8-b995-f25dfe6fb6ae'
    ).then((response) => {
      dispatch(listActions.getListDB(response.data))
    })
  };
  ...
  const _map_2 =() => {
    axios.get('https://run.mocky.io/v3/1446df68-c599-44ba-bf43-7558e5f8761b'
    ).then((response) => {
      dispatch(listActions.getListDB(response.data))
    })
  };
  ...
  const _map_3 =() => {
    axios.get('https://run.mocky.io/v3/183491d8-1499-44ea-9526-8846831dffdb'
    ).then((response) => {
      dispatch(listActions.getListDB(response.data))
    })
  };
  ...
  return(
    <>
      ...
      <div>
        {list.gps !== null? list.result.map((ls,idx) => {
          return(
            <div key={idx}>
              <div>{ls.title}</div>
              <div>{ls.content}</div>
